...
static int g_drawCallCounter = 0;
static DWORD g_lastEnemyDetectionTime = 0;
...
void __stdcall HookedDrawIndexed(ID3D11DeviceContext* pThis, UINT IndexCount, UINT StartIndexLocation, INT BaseVertexLocation) {
    ...
    // Only process every 10th draw call to reduce overhead
    g_drawCallCounter++;
    if (g_drawCallCounter % 10 != 0) {
        return g_originalDrawIndexed(pThis, IndexCount, StartIndexLocation, BaseVertexLocation);
    }

    // Rate limit: Only detect enemies every 150ms
    DWORD currentTime = GetTickCount();
    if (currentTime - g_lastEnemyDetectionTime < 150) {
        return g_originalDrawIndexed(pThis, IndexCount, StartIndexLocation, BaseVertexLocation);
    }
    g_lastEnemyDetectionTime = currentTime;
        ...
        if (vertexBuffer && stride == PLAYER_MODEL_STRIDE) {
            ...
            if (constantBuffer) {
                try {
                    ...
                    // Safely ignore exceptions from GPU buffer access
                }
